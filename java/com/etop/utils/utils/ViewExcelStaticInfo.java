...
*/  
public class ViewExcelStaticInfo extends AbstractExcelView {     
     
	//表头
	public static   final String[] tableHeader = {"宽带标识", "停用年月" ,  "场地地址", "线路类型","链接地址", "运营商" , "接入方式"
    		, "接入宽带", "应缴费用", "营改增", "结算负责人", "付费方式", "结算周期(月)", "支付月", "费用归集"};
       ...
       for(int i=0; i<cellNumber; i++){
        ...
        for (Iterator iter = feeConfirmList.iterator(); iter.hasNext();) {  
           ...
           // sheet.setColumnHidden(1, true);
            if(null != feeConfirmReport.getAddress() && !"".equals(feeConfirmReport.getAddress())){
            	row.createCell(2).setCellValue(feeConfirmReport.getAddress().toString());
            }
            else{
            ...
            if(null != feeConfirmReport.getLinkAddress() && !"".equals(feeConfirmReport.getLinkAddress())){
            	row.createCell(4).setCellValue(feeConfirmReport.getLinkAddress().toString());
            ...
            if(null != feeConfirmReport.getOperator() && !"".equals(feeConfirmReport.getOperator())){
            	row.createCell(5).setCellValue(feeConfirmReport.getOperator().toString());
            	
            }else{
            	row.createCell(5).setCellValue("");
            }
            if(null != feeConfirmReport.getAccessWay() && !"".equals(feeConfirmReport.getAccessWay())){
            	row.createCell(6).setCellValue(feeConfirmReport.getAccessWay().toString());
            ...
            if(null != feeConfirmReport.getBandwidth()  && !"".equals(feeConfirmReport.getBandwidth())){
            	row.createCell(7).setCellValue(feeConfirmReport.getBandwidth().toString());
            	
            }else{
            	row.createCell(7).setCellValue("");
            }
            if(null != feeConfirmReport.getFee()  && !"".equals(feeConfirmReport.getFee()) && ! "0.00".equals(feeConfirmReport.getFee().toString())  ){
            	row.createCell(8).setCellValue(feeConfirmReport.getFee().doubleValue());
            ...
            if(null != feeConfirmReport.getValueAddTax()  && !"".equals(feeConfirmReport.getValueAddTax())
            	...
            	row.createCell(9).setCellValue(feeConfirmReport.getValueAddTax().doubleValue());
...
//            	row.createCell(10).setCellValue(feeConfirmReport.getAgent().toString());
//            }
//            else{
